          ...
          shownItems.shown.map((item) => {
            ...
            const parts = [
              ...
              item.state === 'running'
                ...
                : formatBlockRelativeTime(item.startedAt, now),
            ...
            return (
              <SessionPanelRow
                ...
                meta={
                  <>
                    {/* 실패한 명령은 종료 코드를 먼저, 색으로 구분해 보여 준다. */}
                    {failed ? (
                      <span className="text-[var(--danger-text)]">exit {item.exitCode}</span>
                    ) : null}
                    {failed && parts.length > 0 ? ' · ' : null}
                    {parts.join(' · ')}
                  </>
                }
                actions={actions}
