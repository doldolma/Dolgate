...
function signOfflineLease(serverUrl: string, userId: string, expiresAt: Date): AuthSession['offlineLease'] {
  ...
  return {
    ...
    verificationPublicKeyPem: publicKey.export({ type: 'spki', format: 'pem' }).toString()
  };
}
...
function createSession(serverUrl: string, userId = 'user-1'): AuthSession {
  return {
    ...
    offlineLease: signOfflineLease(serverUrl, userId, new Date(Date.now() + 72 * 60 * 60 * 1000)),
...
async function createService(serverUrl = 'https://ssh.doldolma.com') {
  ...
  return {
    secretStore,
    service: new AuthService(secretStore, configService as never, settings as never)
  };
}
...
describe('AuthService offline bootstrap', () => {
  it('enters offline-authenticated when refresh fails but a valid offline lease is cached', async () => {
    ...
    expect(Math.abs(new Date(state.offline?.expiresAt ?? 0).getTime() - new Date(session.offlineLease.expiresAt).getTime())).toBeLessThan(
      1_500
    );
    expect(service.getVaultKeyBase64()).toBe(session.vaultBootstrap.keyBase64);
    expect(() => service.getAccessToken()).toThrow('오프라인 모드에서는 서버 연결이 필요한 기능을 사용할 수 없습니다.');
  });
  ...
  it('reconnects back to authenticated when retryOnline succeeds', async () => {
    ...
    await secretStore.save('auth:refresh-token', cachedSession.tokens.refreshToken);
    await secretStore.save(
      'auth:offline-session-cache',
      JSON.stringify({
        serverUrl: `${serverUrl}/`,
    ...
    expect(state.session?.tokens.accessToken).toBe(refreshedSession.tokens.accessToken);
  });

  it('rejects stale offline cache when the configured server URL changed', async () => {
    const { service, secretStore } = await createService('https://new.example.com');
