// vnc-core(services/vnc-core, Rust) 릴리스 빌드.
//
// build-vnc-core.cjs 와 같은 규칙·같은 출력 자리를 쓴다:
// release/resources/<platform>/<arch>/bin/.
//
// 크로스컴파일이 안 되는 이유도 같다. vnc-core 는 **vendored OpenSSL**(C 라이브러리)을 함께
// 빌드한다 — VeNCrypt 의 TLSVnc 가 익명 DH 를 쓰고 rustls 는 그것을 지원하지 않아서다. 그래서
// 타깃용 C 툴체인이 필요하고, 릴리스 워크플로가 플랫폼별 네이티브 러너에서 도는 것에 의존한다.
//
// 유일한 예외가 macOS universal 이다. Apple 타깃은 같은 시스템 clang 으로 둘 다 빌드되므로
// rustup 타깃만 추가하면 lipo 로 합칠 수 있다.
//
// vendored OpenSSL 은 perl 과 make 를 요구한다 — 세 러너 모두 갖고 있지만, 없는 환경에서는
// cargo 가 그 이유를 그대로 말해 준다.
...
function run(command, args, options = {}) {
  ...
  if (result.error && result.error.code === 'ENOENT') {
    throw new Error(
      `${command} not found. Install the Rust toolchain (https://rustup.rs) to build vnc-core.`
    );
  }
 ...
 *
 * ssh-core 는 GOARCH 만 바꾸면 되지만 vnc-core 는 OpenSSL(C)을 소스에서 빌드하므로 타깃용 C
...
function BINARY_NAME() {
  return process.platform === 'win32' ? 'vnc-core.exe' : 'vnc-core';
}
...
const SERVICE_DIR = path.join(REPO_ROOT, 'services', 'vnc-core');

function buildDarwinUniversal(releaseRoot, targetRoot) {
  const tempRoot = path.join(releaseRoot, 'tmp', 'vnc-core', 'darwin');
  const outputPath = path.join(targetRoot, 'vnc-core');
...
function buildFor(targetRoot, expectedPlatform, target, env = {}) {
  if (process.platform !== expectedPlatform) {
    throw new Error(
      `vnc-core for ${expectedPlatform} must be built on a ${expectedPlatform} host; this one is ${process.platform}. ` +
        'It builds OpenSSL from source, so it does not cross-compile the way ssh-core does.'
    );
  }
...
function main() {
  ...
  if (!platform || !arch) {
    throw new Error('Usage: node ./scripts/build-vnc-core.cjs <platform> <arch>');
  }
  ...
  throw new Error(`Unsupported vnc-core release target: ${platform}/${arch}`);
}
