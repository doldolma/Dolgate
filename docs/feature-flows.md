...
- SFTP(원격) 패널에서 텍스트 파일을 더블클릭하거나 우클릭 **편집**으로 인앱 에디터를 엽니다.
- 편집 가능 조건: 디렉터리가 아니고, 설정한 최대 크기(기본 5MB) 이하이며, 바이너리가 아닐 것. 부적합한 파일은 조용히 열리지 않습니다.
- 흐름:
  - `sftpReadFile`로 내용을 메모리에 읽어옵니다. (크기 캡과 NUL 바이트 기반 바이너리 판별은 `ssh-core`가 수행)
  - 에디터에서 수정하면 dirty 상태로 표시되고, `Cmd/Ctrl+S` → `sftpWriteFile`로 저장합니다.
  - 일반 파일은 temp + rename으로 원자적 저장, 권한이 없으면 sudo(`install`)로 저장합니다.
- 충돌: 열 때의 size/mtime 스냅샷을 저장 직전 원격과 비교해, 바뀌었으면 다시 불러오기/덮어쓰기를 선택합니다.
- sudo: 권한 거부 시 sudo 비밀번호를 요청하며, 비밀번호는 stdin으로만 전달됩니다.
