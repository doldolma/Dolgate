// web/*.html(한국어)에서 web/en/*.html(영어)을 만든다.
//
// 한국어 원문은 마크업 자체이고 영어는 페이지별 i18n JSON 에만 있다 — 같은 문구를 두 파일에
...
// koLangHref 는 영어 페이지의 언어 토글이 가리킬 한국어 페이지 경로다(en/ 기준 상대 경로).
const PAGES = [
  { source: "index.html", dict: "i18n.en.json", out: join("en", "index.html"), koLangHref: "../" },
  { source: "tailscale.html", dict: "i18n.en.tailscale.json", out: join("en", "tailscale.html"), koLangHref: "../tailscale.html" },
  { source: "aws-ssm.html", dict: "i18n.en.aws-ssm.json", out: join("en", "aws-ssm.html"), koLangHref: "../aws-ssm.html" },
];

let failed = false;

for (const page of PAGES) {
  const html = await readFile(join(webDir, page.source), "utf8");
  const dict = JSON.parse(await readFile(join(webDir, page.dict), "utf8"));
  ...
  // 3) canonical 은 자기 자신을 가리켜야 한다 — /Dolgate/<tail> 을 /Dolgate/en/<tail> 로.
  //    hreflang 쌍은 두 언어 페이지에서 동일한 목록이므로 그대로 둔다.
  replaceOnce(
  ...
  // 4) 언어 링크는 반대쪽(한국어) 페이지를 가리킨다.
  replaceOnce(
    /<a class="btn icon-btn" id="lang-link"[^>]*>[^<]*<\/a>/,
    `<a class="btn icon-btn" id="lang-link" href="${page.koLangHref}" hreflang="ko" title="한국어" aria-label="한국어">한</a>`,
    "언어 링크",
  );
  ...
  // 페이지 사이 이동(./tailscale.html 등)과 앵커(./#download)는 en/ 안에 같은 이름의
  // 생성물이 있으므로 그대로 둔다 — en 페이지끼리 연결된다.

  if (problems.length) {
    failed = true;
    console.error(`web/en 생성 실패 (${page.source}):`);
  ...
  out = out.replace(
    /^<!doctype html>/i,
    `<!doctype html>\n<!-- 생성된 파일 — web/${page.source} + web/${page.dict} 에서 web/build-en.mjs 가 만든다. 직접 고치지 말 것. -->`,
  );

  await mkdir(dirname(join(webDir, page.out)), { recursive: true });
  await writeFile(join(webDir, page.out), out);
  console.log(
    `web/${page.out} 생성 — 번역 ${Object.keys(dict.keys).length}곳, 런타임 문구 ${Object.keys(dict.runtime).length}개`,
  );
}
