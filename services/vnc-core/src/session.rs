...
///
/// **이 목록이 "아직 안 되는 것" 의 기록이다.** 여기 오는 서버를 만났다면 아래가 무엇을 만들어야
/// 하는지다. 각각을 왜 미뤘는지도 함께 적는다 — 우선순위를 다시 정할 때 그 판단 근거가 필요하다.
///
/// - **RSA-AES(5·6·129·130)**: RSA 로 서버 키를 인증하고 AES-EAX 로 세션을 암호화한다. 우리가
///   가진 것 중 가장 만들 만한 후보다(openssl 이 이미 있다). 미룬 이유는 TigerVNC 기본값이
///   `TLSVnc,VncAuth` 라서다 — 관리자가 `SecurityTypes` 에 명시해야 켜지는 옵트인이다.
/// - **Tight 보안 확장(16)**: TightVNC 계열의 인증 협상 감싸기. 그 안에서 다시 방식을 고른다.
/// - **TLS(18)**: VeNCrypt 이전의 옛 방식. 지금 이것만 쓰는 서버는 드물다.
/// - **SASL(20)**: 방식이 하나가 아니라 GSSAPI·SCRAM 등 메커니즘 협상이다. 붙는 서버 하나를
///   만나기 전에 만들면 어느 메커니즘을 지원할지부터 추측이 된다.
/// - **macOS 독자(33·35·36)**: 공개 규격이 없다. macOS 는 이것과 함께 ARD(30)도 제시하고 우리는
///   그쪽으로 붙으므로, 실제로 막히는 경우는 관리자가 ARD 를 끈 구성뿐이다.
///
/// 여기 없는 것도 하나 있다: **맨몸 Plain(256)** 은 구현했지만 일부러 거부한다(`vencrypt` 참고) —
/// TLS 없이 비밀번호가 평문으로 나간다.
