...
typedef struct {
    ...
    const char *host;
    /* Actual host:port dial endpoint; null/empty means host + port. */
    ...
    /* 0 for default, otherwise 16 or 32. */
    uint8_t color_depth;
    /* Optional JSON array of {label,path,readOnly}. */
...
/* Joins the worker; do not call synchronously from a callback. */
