...
mod openh264_impl {
    ...
    impl OpenH264Decoder {
        ...
        ///
        /// PATCH (Dolgate): the wire data is usually already Annex B — pass it through.
        ///
        /// MS-RDPEGFX carries the H.264 bitstream in Annex B form, with `00 00 00 01` (or
        /// `00 00 01`) start codes. Reading that as a 4-byte length prefix yields a NAL of length
        /// 1, and every NAL after it is cut at the wrong place, so the decoder is fed noise and
        /// never returns a picture. That is silent: nothing errors, the screen simply never
        /// updates from H.264.
