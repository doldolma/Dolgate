...
fn decode_vbar<'a>(src: &mut ReadCursor<'a>, band_height: u16) -> DecodeResult<VBar<'a>> {
    ...
    //
    // PATCH (Dolgate): 상류는 이 두 필드의 자리를 반대로 읽는다.
    //
    // MS-RDPEGFX 2.2.4.1.1.2.1.1.3 의 예제(Example 4)가 자리를 못 박아 준다:
    //   바이트 `00 0f` -> vBarHeader = 0x0F00, shortVBarYOn = 0x00, shortVBarYOff = 0x0F
    // 즉 위쪽 6비트(13:8)가 yOff 이고 아래 8비트(7:0)가 yOn 이다. 상류처럼 읽으면 이 예제가
    // yOn=60, yOff=0 이 되어 "shortVBarYOff < shortVBarYOn" 으로 튕기고, 그 자리에서 커서가
    // 어긋나 뒤따르는 V-Bar 들이 전부 엉뚱하게 해석된다(그 다음은 "V-bar cache miss on hit").
    //
    //   bits 13:8 = shortVBarYOff (6 bits): Short V-Bar 가 끝나는 행
    //   bits 7:0  = shortVBarYOn  (8 bits): Short V-Bar 가 시작하는 행
    // 픽셀 수 = shortVBarYOff - shortVBarYOn
...
mod tests {
    ...
    fn decode_vbar_short_cache_miss() {
        ...
        // Layout is yOff in bits 13:8 and yOn in bits 7:0 (see the spec example below).
        let y_on: u16 = 2;
        let y_off: u16 = 5;
    ...
    /// PATCH (Dolgate): the first V-Bar of MS-RDPEGFX "Example 4", byte for byte.
    ///
    /// The wire bytes are `00 0f`, and the specification decodes them as
    /// `vBarHeader = 0x0F00`, `shortVBarYOn = 0x00`, `shortVBarYOff = 0x0F`, followed by 15
    /// pixels. Reading yOn from bits 13:6 instead turns this into `yOn = 60, yOff = 0`, which
    /// is rejected as "shortVBarYOff < shortVBarYOn" — and every V-Bar after it is then read
    /// from the wrong offset.
    #[test]
    fn decode_vbar_short_cache_miss_matches_the_specification_example() {
        let mut data = vec![0x00, 0x0f];
        data.extend_from_slice(&[0xff; 15 * 3]);

        let mut cursor = ReadCursor::new(&data);
        let vbar = decode_vbar(&mut cursor, 15).unwrap();

        match vbar {
            VBar::ShortCacheMiss(miss) => {
                assert_eq!(miss.y_on, 0x00);
                assert_eq!(miss.y_off_delta, 0x0f);
                assert_eq!(miss.pixel_data.len(), 15 * 3);
            }
            _ => panic!("expected ShortCacheMiss"),
        }
        assert_eq!(cursor.len(), 0, "the whole V-Bar must be consumed");
    }
