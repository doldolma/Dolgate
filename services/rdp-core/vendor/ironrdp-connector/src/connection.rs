            ...
            optional_data: ClientCoreOptionalData {
                ...
                early_capability_flags: {
                    let mut early_capability_flags =
                        ...
                        // PATCH (Dolgate): 그래픽 파이프라인(EGFX)을 쓰겠다고 선언한다.
                        //
                        // 상류는 이 비트를 세우지 않는다. 서버는 지원한다고 답해도
                        // (RDP_NEG_RSP 의 DYNVC_GFX_PROTOCOL_SUPPORTED) 클라이언트가 요청하지
                        // 않으면 Microsoft::Windows::RDS::Graphics 채널을 열지 않는다.
                        // 스크롤의 화면 영역 복사와 H.264 가 전부 그 채널 위에 있다.
                        //
                        // 요청하지 않기로 했으면 비트도 세우지 않는다. 세워 놓고 채널을 거절하면
                        // 서버가 열려다 실패하는 왕복이 한 번 더 생긴다.
