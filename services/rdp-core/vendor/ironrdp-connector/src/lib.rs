...
pub struct Config {
    ...
    /// PATCH (Dolgate): monitor layout to declare at connect time.
    ///
    /// Upstream hardcodes `monitor: None` in the GCC blocks, so a client cannot tell the server it
    /// has several displays. When this is non-empty it becomes the
    /// [`TS_UD_CS_MONITOR`](gcc::ClientMonitorData) block.
    ///
    /// Coordinates are in the declaration space: the primary monitor sits at (0, 0) and the others
    ...
    /// `Microsoft::Windows::RDS::Graphics` channel — the surface copies that make scrolling cheap,
    /// and H.264, all live there.
    ///
    /// It is a request, not a capability we can take back: once the channel is open the server
    /// sends its graphics there and nowhere else. A client that cannot decode what arrives is left
    /// with a blank screen, so this is off unless a graphics pipeline channel is registered too.
