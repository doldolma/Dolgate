        ...
        match sub.codec_id {
            ...
            SubcodecId::Rlex => {
                ...
                for seg in &rlex.segments {
                    ...
                    // PATCH (Dolgate): 스위트는 startIndex 를 포함하지 않는다.
                    //
                    // MS-RDPEGFX 2.2.4.1.1.3.1.3: "Each index represents one pixel preceding the
                    // stopIndex and starting from stopIndex - suiteDepth". 즉 스위트 픽셀 수는
                    // 정확히 suiteDepth 개이고, startIndex 색은 바로 위 런에서 이미 칠했다.
                    //
                    // 상류처럼 startIndex 부터 세면 세그먼트마다 픽셀이 하나씩 밀린다. suiteDepth
                    // 가 0 인(=순수 런) 세그먼트에서도 한 픽셀이 더 나가므로 화면이 통째로
                    // 어긋나고, 결국 "suite exceeds region pixel count" 로 디코딩이 멈춘다.
                    for palette_idx in seg.start_index.saturating_add(1)..=seg.stop_index {
                        if px >= region_pixels {
            ...
            SubcodecId::NsCodec => {
                // PATCH (Dolgate): 상류는 여기서 아무것도 하지 않는다 — 자기 인코더가 NSCodec
                // 타일을 만들지 않기 때문이다. 그런데 Windows 서버는 사진 같은 영역을 이
                // 서브코덱으로 보내고, 그러면 오류도 로그도 없이 그 자리가 빈 채로 남았다.
                let pixels = crate::nscodec::decode(sub.bitmap_data, sub.width, sub.height)?;
                ...
                for row in 0..h {
                    for col in 0..w {
                        ...
                        let dst_idx = (y * sw + x) * 4;
                        if dst_idx + 3 >= output.len() || src_idx + 3 >= pixels.len() {
                            continue;
                        }
...
mod tests {
    ...
    /// PATCH (Dolgate): an RLEX segment writes `runLength + suiteDepth` pixels, not one more.
    ///
    /// MS-RDPEGFX 2.2.4.1.1.3.1.3 says each suite index "represents one pixel preceding the
    /// stopIndex and starting from stopIndex - suiteDepth", so `startIndex` itself belongs to the
    /// run, not to the suite. Counting it twice shifts every segment by a pixel — and a region
    /// that fits exactly, like this one, overruns and stops decoding altogether.
    #[test]
    fn rlex_suite_does_not_repeat_the_run_colour() {
        let mut decoder = ClearCodecDecoder::new();

        // palette_count = 4 -> stopIndex takes 2 bits, suiteDepth the other 6.
        let palette: [[u8; 3]; 4] = [
            [0x11, 0x11, 0x11],
            [0x22, 0x22, 0x22],
            [0x33, 0x33, 0x33],
        ...
        let mut rlex = vec![4u8];
        for entry in &palette {
            rlex.extend_from_slice(entry);
        }
        // stopIndex = 3, suiteDepth = 2 -> startIndex = 1. Run of 2, then the suite (2 pixels).
        rlex.push((2 << 2) | 3);
        rlex.push(2);
        ...
        subcodec_data.extend_from_slice(&0u16.to_le_bytes()); // x_start
        subcodec_data.extend_from_slice(&0u16.to_le_bytes()); // y_start
        subcodec_data.extend_from_slice(&4u16.to_le_bytes()); // width
        subcodec_data.extend_from_slice(&1u16.to_le_bytes()); // height
        subcodec_data.extend_from_slice(&u32::try_from(rlex.len()).unwrap().to_le_bytes());
        subcodec_data.push(0x02); // SubcodecId::Rlex
        subcodec_data.extend_from_slice(&rlex);

        let mut stream = vec![0x00, 0x00]; // flags, seq
        stream.extend_from_slice(&0u32.to_le_bytes()); // residual
        stream.extend_from_slice(&0u32.to_le_bytes()); // bands
        stream.extend_from_slice(&u32::try_from(subcodec_data.len()).unwrap().to_le_bytes());
        stream.extend_from_slice(&subcodec_data);

        let pixels = decoder.decode(&stream, 4, 1).unwrap();

        // Run: palette[1] twice. Suite: palette[2], palette[3] — palette[1] is not repeated.
        assert_eq!(&pixels[0..4], &[0x22, 0x22, 0x22, 0xFF]);
        assert_eq!(&pixels[4..8], &[0x22, 0x22, 0x22, 0xFF]);
        assert_eq!(&pixels[8..12], &[0x33, 0x33, 0x33, 0xFF]);
        assert_eq!(&pixels[12..16], &[0x44, 0x44, 0x44, 0xFF]);
    }
