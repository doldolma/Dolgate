...
// ProbeControlMap sends a non-streaming map request using the current node
// identity. It is useful for control servers that do not terminate an existing
// streaming map response when an enrolled node is deleted.
func (c *Auto) ProbeControlMap(ctx context.Context) error {
	c.mu.Lock()
	if c.closed {
		c.mu.Unlock()
	...
	if c.paused {
		c.mu.Unlock()
		return errors.New("control client is paused")
	}
	if !c.loggedIn {
		c.mu.Unlock()
		return errors.New("control client is not logged in")
	}
	mapCtx := c.mapCtx
	direct := c.direct
	c.mu.Unlock()
	...
	stop := context.AfterFunc(mapCtx, cancel)
	defer func() {
		stop()
		cancel()
	}()
	return direct.SendUpdate(probeCtx)
}
