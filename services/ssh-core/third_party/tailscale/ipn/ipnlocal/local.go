...
type LocalBackend struct {
	...
	controlError *ipn.ControlError    // last structured control-plane error, or nil
...
func (b *LocalBackend) setControlClientStatusLocked(c controlclient.Client, st controlclient.Status) {
	...
	if st.Err != nil {
		...
		if controlErr := controlErrorFromStatusError(st.Err); controlErr != nil {
			b.controlError = controlErr
			b.sendLocked(ipn.Notify{ControlError: controlErr})
		}
	...
	if st.NetMap != nil && b.controlError != nil {
		b.controlError = nil
		b.sendLocked(ipn.Notify{ControlError: new(ipn.ControlError)})
	}
...
func controlErrorFromStatusError(err error) *ipn.ControlError {
	var mapErr *controlclient.MapResponseError
	if !errors.As(err, &mapErr) || mapErr.Kind != controlclient.MapResponseErrorNodeNotFound {
		return nil
	}
	return &ipn.ControlError{
		Kind:       ipn.ControlErrorNodeNotFound,
		StatusCode: mapErr.StatusCode,
		Message:    mapErr.Error(),
	}
}

// ProbeControlMap verifies the current node identity with a fresh,
// non-streaming map request. Some control servers leave the streaming map
// response open after deleting a node, so waiting for that stream alone cannot
// observe the deletion.
func (b *LocalBackend) ProbeControlMap(ctx context.Context) (*ipn.ControlError, error) {
	b.mu.Lock()
	cc := b.ccAuto
	b.mu.Unlock()
	if cc == nil {
		return nil, errors.New("control client is not ready")
	}

	err := cc.ProbeControlMap(ctx)
	controlErr := controlErrorFromStatusError(err)
	...
	if controlErr != nil {
		b.controlError = controlErr
		b.sendLocked(ipn.Notify{ControlError: controlErr})
	} else if err == nil && b.controlError != nil {
		b.controlError = nil
		b.sendLocked(ipn.Notify{ControlError: new(ipn.ControlError)})
	}
	return controlErr, err
...
func (b *LocalBackend) WatchNotificationsAs(ctx context.Context, actor ipnauth.Actor, mask ipn.NotifyWatchOpt, onWatchAdded func(), fn func(roNotify *ipn.Notify) (keepGoing bool)) {
	...
	if mask&initialBits != 0 {
		...
		if mask&ipn.NotifyInitialState != 0 {
			...
			if b.controlError != nil {
				controlErr := *b.controlError
				ini.ControlError = &controlErr
			}
