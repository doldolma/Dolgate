...
type Notify struct {
	...
	// ControlError, when non-nil, reports a structured control-plane failure.
	// An empty Kind clears the previously reported error.
...
// ControlErrorKind identifies a control-plane failure that requires a
// different recovery policy from an ordinary network or polling error.
type ControlErrorKind string

const (
	// ControlErrorNodeNotFound means the control plane no longer recognizes the
	// node identity persisted by this client.
	ControlErrorNodeNotFound ControlErrorKind = "nodeNotFound"
)

// ControlError is the wire representation of a structured control-plane error.
