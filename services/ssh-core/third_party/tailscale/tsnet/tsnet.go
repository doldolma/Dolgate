...
// ProbeControlMap verifies that the control server still recognizes this
// server's current node identity.
func (s *Server) ProbeControlMap(ctx context.Context) (*ipn.ControlError, error) {
	if err := s.Start(); err != nil {
		return nil, err
	}
	return s.lb.ProbeControlMap(ctx)
}
