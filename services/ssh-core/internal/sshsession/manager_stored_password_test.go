...
func TestApplyStoredPassword(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got := applyStoredPassword(test.response, stored, test.promptCount)
...
func TestApplyStoredPasswordWithoutStoredValue(t *testing.T) {
	got := applyStoredPassword(interactiveResponse{
		Responses:             []string{"typed"},
		StoredPasswordIndexes: []int{0},
	}, "", 1)
