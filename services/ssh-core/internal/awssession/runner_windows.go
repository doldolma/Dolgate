...
func startPlatformAWSRunner(payload protocol.AWSConnectPayload, runtime awsCommandRuntime) (sessionRunner, error) {
	...
	inputWriter := os.NewFile(uintptr(inputPipe[1]), "aws-ssm-stdin")
	outputReader := os.NewFile(uintptr(outputPipe[0]), "aws-ssm-stdout")
...
func ensureConPTYSupport() error {
	...
	if version.MajorVersion < 10 || version.BuildNumber < minimumConPTYBuild {
		return fmt.Errorf("Windows 10 1809+/Server 2019+ 이상에서만 AWS SSM interactive session을 지원합니다.")
	}
	return nil
}
