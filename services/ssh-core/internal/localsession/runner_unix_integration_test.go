...
func TestResolveUnixShellExecutableWithLookupPrefersEnvThenFallbacks(t *testing.T) {
	resolved, err := resolveUnixShellExecutableWithLookup("/custom/shell", func(candidate string) bool {
		return candidate == "/custom/shell"
	})
	if err != nil {
		t.Fatalf("expected shell from env, got error: %v", err)
	}
	if resolved != "/custom/shell" {
		t.Fatalf("resolved shell = %q", resolved)
	}

	resolved, err = resolveUnixShellExecutableWithLookup("/missing/shell", func(candidate string) bool {
