...
- **종단간 암호화 동기화 (zero-knowledge)** — 동기화 암호 설정 또는 기존 계정의 E2EE 전환 후에는 복호화 키가 사용자 암호로만 풀리며, 서버가 원문 키를 갖지 않습니다.
...
- 종단간 암호화(E2EE) — 동기화 암호 설정 또는 기존 계정의 E2EE 전환 후에는 호스트·자격 증명·스니펫 등이 기기에서 암호화되어 서버에는 암호문만 저장됩니다
