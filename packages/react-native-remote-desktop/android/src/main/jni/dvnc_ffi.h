...
/**
 * C header for vnc-core FFI (mirrors services/vnc-core/src/ffi.rs).
 * Android JNI bridge includes this to call into the Rust cdylib.
 */
...
typedef void (*DvncOnConnected)(void* user_data, uint16_t desktop_width, uint16_t desktop_height, const char* name);
typedef void (*DvncOnResized)(void* user_data, uint16_t desktop_width, uint16_t desktop_height);
...
typedef void (*DvncOnFrame)(void* user_data, uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint8_t* pixels, uint32_t pixels_len);
typedef void (*DvncOnCursor)(void* user_data, uint16_t hotspot_x, uint16_t hotspot_y, uint16_t width, uint16_t height, const uint8_t* rgba, uint32_t rgba_len);
...
/* Public C ABI */
int32_t dvnc_session_create(const DvncCallbacks* callbacks, DvncSessionHandle* out_handle);
int32_t dvnc_session_start(DvncSessionHandle handle, const DvncConnectConfig* config);
...
int32_t dvnc_session_pointer_button(DvncSessionHandle handle, uint8_t button, uint8_t pressed, uint16_t x, uint16_t y);
int32_t dvnc_session_pointer_scroll(DvncSessionHandle handle, uint8_t vertical, int16_t delta, uint16_t x, uint16_t y);
...
int32_t dvnc_session_request_desktop_size(DvncSessionHandle handle, uint16_t width, uint16_t height);
