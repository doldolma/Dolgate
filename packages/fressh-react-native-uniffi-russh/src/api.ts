...
type RusshApi = {
  ...
  connectSftp: (opts: ConnectSftpOptions) => Promise<SftpConnection>;
  generateKeyPair: (
...
} as const satisfies Record<GeneratedRussh.SftpEntryKind, SftpEntryKind>;
...
type GeneratedSftpConnection = {
  getInfo: () => {
    connectionId: string;
    connectionDetails: GeneratedRussh.ConnectionDetails;
    createdAtMs: number;
  ...
  listDirectory: (
    path: string,
    opts?: { signal: AbortSignal },
  ) => Promise<GeneratedRussh.SftpListing>;
  readFileChunk: (
    path: string,
    offset: number,
    length: number,
    opts?: { signal: AbortSignal },
  ) => Promise<GeneratedRussh.SftpReadChunk>;
  writeFileChunk: (
    path: string,
    offset: number,
    data: ArrayBuffer,
  ...
  delete_: (path: string, opts?: { signal: AbortSignal }) => Promise<void>;
  close: (opts?: { signal: AbortSignal }) => Promise<void>;
};

type GeneratedSftpModule = typeof GeneratedRussh & {
  connectSftp?: (
    options: {
      connectionDetails: GeneratedRussh.ConnectionDetails;
      onConnectionProgressCallback?: {
        onChange: (status: GeneratedRussh.SshConnectionProgressEvent) => void;
      };
    ...
    opts?: { signal?: AbortSignal },
  ) => Promise<GeneratedSftpConnection>;
};

function wrapSftpConnection(conn: GeneratedSftpConnection): SftpConnection {
  const info = conn.getInfo();
  const asyncOpts = (opts?: { signal?: AbortSignal }) =>
  ...
  return {
    ...
    readFileChunk: (path, offset, length, opts) =>
      conn.readFileChunk(path, offset, length, asyncOpts(opts)),
    writeFileChunk: (path, offset, data, opts) =>
      conn.writeFileChunk(path, offset, data, asyncOpts(opts)),
    mkdir: (path, opts) => conn.mkdir(path, asyncOpts(opts)),
    rename: (sourcePath, targetPath, opts) =>
      conn.rename(sourcePath, targetPath, asyncOpts(opts)),
    chmod: (path, permissions, opts) =>
      conn.chmod(path, permissions, asyncOpts(opts)),
...
async function connectSftp({
  ...
  onDisconnected,
  ...options
}: ConnectSftpOptions): Promise<SftpConnection> {
  const generated = GeneratedRussh as GeneratedSftpModule;
