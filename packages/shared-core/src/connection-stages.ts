...
//
// 진행을 한 줄 문구로 보여주면 새 단계가 앞 단계를 덮어써서, 지나간 것은 사라지고 지금 것만 남는다.
// 단계가 빨리 지나가면 사용자는 아무것도 못 본 것과 같고, 실패했을 때 어디까지 갔는지도 알 수 없다 —
// Tailscale 때문에 못 붙은 것인지 SSH 가 거절한 것인지 구분할 방법이 없었다.
//
// 그래서 지나간 단계를 남긴다. 각 단계는 스스로 상태를 갖고, 실패는 그 단계에 붙는다.
//
// **여기 있는 이유.** 데스크톱과 모바일이 같은 화면을 보여줘야 한다. tailnet 계층의 판정에는 쉽게
// 얻은 것이 아닌 규칙들이 들어 있어서(등록과 동기화를 나눠 보는 것, degraded 를 실패로 그리지 않는
// 것, 링크를 기다리는 구간을 따로 말하는 것) 두 벌로 두면 한쪽만 고쳐진다.
//
// 문구는 돌려주지 않는다 — 이 패키지는 UI 언어를 결정할 수 없으므로 i18n 키와 값만 주고, 각 앱이
// 자기 카탈로그에서 문구를 찾는다(connection-failure.ts 와 같은 규칙).

import { isAwsEc2WindowsPlatform } from "./models";
import type { HostRecord, TerminalConnectionStage } from "./models";
import type { ConnectionFailureLayer } from "./connection-failure";

/**
 * 단계 계산이 tailnet 상태에서 **읽는 것만** 적은 것.
 *
 * 각 앱의 상태 타입을 여기로 끌어오지 않는다 — 데스크톱은 IPC 페이로드에서, 모바일은 엔진
 * 이벤트에서 오고 둘 다 이 화면이 쓰는 것보다 넓다. 필요한 필드만 적어 두면 구조가 맞는 쪽이
 * 그대로 들어온다.
 *
 * 전부 선택이다. 어느 쪽 타입에서 필수가 아닌 필드를 여기서 필수로 적으면 그 앱은 넘길 수 없다.
 */
export interface TailnetStagePeer {
  hostName?: string;
  dnsName?: string;
  ips?: readonly string[];
  /** 이 기기가 광고한 서브넷(CIDR). 사내 랜 주소는 기기가 아니라 이것을 통해 닿는다. */
...
export interface TailnetStageStatus {
  state?: string | null;
  ...
  loginError?: string;
  authUrl?: string;
  /** 링크를 기다리다 노드를 다시 세운 횟수. */
  restarts?: number;
  health?: readonly string[];
  error?: string;
  peers?: readonly TailnetStagePeer[];
}
...
/** 앱이 자기 카탈로그에서 찾을 문구. */
export interface ConnectionStageText {
  key: string;
 ...
 * 번역하지 않고 그대로 보여줄 조각.
 *
 * 백엔드·서버가 준 원문(로그인 거부 이유, 헬스 경고, tailnet 오류)이다. 이것을 키로 바꾸려 하면
 * 우리가 모르는 문장을 분류하게 되고, 분류에 없는 것은 사라진다.
 */
export interface ConnectionStageRawText {
  text: string;
}
...
export interface ConnectionStage {
  ...
  label: ConnectionStageText;
  state: ConnectionStageState;
  /** 그 단계에 대해 더 말할 것. 조각을 순서대로 이어 붙여 보여준다. */
  detail?: ConnectionStageDetailPart[];
}

/**
 * 호스트 계층이 실제로 무엇을 하는지.
 *
 * 이 구분이 없으면 무엇에 붙든 "호스트 키 확인 → SSH 연결" 이 뜬다. 로컬 셸에는 호스트 키도 SSH 도
 * 없고, RDP·시리얼·ECS Exec 은 SSH 를 쓰지 않는다 — 없는 관문을 통과하는 것처럼 보이면 어디서
 * 막혔는지 읽을 수 없다.
 */
...
/**
 * 연결 하나의 진행 상태. **이 화면이 그 연결에 대해 아는 것의 전부다.**
 *
 * 예전에는 여기에 데스크톱의 TerminalTab 이 그대로 들어와서, 같은 단계 화면을 터미널 밖(포워딩·
 * 컨테이너·공개키 설치)에서도, 다른 플랫폼에서도 쓸 수 없었다. 필요한 네 가지만 받으면 무엇이든
 * 환산해 넣을 수 있다.
 */
...
/** 있는 조각만 남긴다. 화면은 이것을 ' · ' 로 이어 붙인다. */
function details(
  ...parts: Array<ConnectionStageDetailPart | string | undefined | null>
): ConnectionStageDetailPart[] | undefined {
  const kept: ConnectionStageDetailPart[] = [];
...
function transportStageLabel(transport: ConnectionTransport): ConnectionStageText | null {
  switch (transport) {
    case "local":
      return null;
    case "serial":
      return text("connectStages.serial");
    case "ecs-exec":
      return text("connectStages.ecsExec");
    case "rdp":
      return text("connectStages.rdp");
    case "vnc":
      return text("connectStages.vnc");
    case "ssm":
      return text("connectStages.ssm");
    default:
      return text("connectStages.ssh");
  }
}
...
export function resolveTailscaleStages(
  status: TailnetStageStatus | undefined,
  failed: boolean,
  target?: string,
): ConnectionStage[] {
  ...
  const nodeStage: ConnectionStage = {
    id: "tailscale-node",
    group: "tailscale",
    label: text("connectStages.tailscaleNode"),
    state: started ? "done" : failed ? "failed" : "active",
    detail: details(status?.backendState),
  };
  ...
  const registrationStage: ConnectionStage = {
    ...
    group: "tailscale",
    label: text("connectStages.tailscaleRegistration"),
    state:
      loginError || expired
        ? "failed"
        : identityInvalid
    ...
    detail: loginError
      ? // 백엔드가 준 이유를 그대로 붙인다. "실패했습니다" 만으로는 키를 고쳐야 하는지 알 수 없다.
        details(text("connectStages.tailscaleLoginRejectedDetail"), loginError)
      : expired
        ? details(text("connectStages.tailscaleExpiredDetail"))
        : identityInvalid
          ? details(text("connectStages.tailscaleIdentityInvalidDetail"))
          : status?.keyExpiry
            ? details(
                text("connectStages.tailscaleKeyExpiryDetail", {
                  at: formatExpiry(status.keyExpiry),
                }),
              )
            : undefined,
  };
  ...
  const authStage: ConnectionStage = {
    id: "tailscale-auth",
    group: "tailscale",
    label: text("connectStages.tailscaleAuth"),
    state:
    ...
    detail: loginError
      ? undefined
      : state === "needsApproval"
        ? details(text("connectStages.tailscaleApprovalDetail"))
        : state === "needsAuth"
          ? status?.authUrl
            ? details(text("connectStages.tailscaleBrowserDetail"))
            : // 링크가 오지 않으면 코어가 노드를 다시 세워 등록을 처음부터 밟는다. 그 사실을
              // 여기서 말해야 한다 — 재시작 전후의 상태가 완전히 같아서, 말하지 않으면 화면은
              // 아무 일도 없는 것처럼 보이고 사용자는 멈춘 줄 안다.
              details(
                text("connectStages.tailscaleLinkDetail"),
                status?.restarts
                  ? text("connectStages.tailscaleRestartDetail", {
  ...
  const networkStage: ConnectionStage = {
    ...
    group: "tailscale",
    label: text("connectStages.tailscaleSync"),
    state:
      status?.online === true
        ? "done"
        : degraded
    ...
    detail:
      status?.online === true && peers > 0
        ? details(text("connectStages.tailscalePeersDetail", { count: peers }))
        : degraded
          ? details(text("connectStages.tailscaleDegradedDetail"))
          : status?.online === false && status.state === "running"
            ? details(text("connectStages.tailscaleStaleDetail"))
            : undefined,
  };
  ...
  for (const [index, warning] of (status?.health ?? []).entries()) {
    stages.push({
      ...
      group: "tailscale",
      label: text("connectStages.tailscaleHealth"),
      state: "warn",
  ...
  if (status?.error) {
    stages.push({
      id: "tailscale-error",
      group: "tailscale",
      label: text("connectStages.tailscaleError"),
      state: "failed",
      detail: details(status.error),
    });
  }
...
export function resolveConnectionStages(
  ...
  if (checksHostKey) {
    stages.push({
      id: "host-key",
      group: "host",
      label: text("connectStages.hostKey"),
      state:
      ...
      detail:
        failureLayer === "hostKey"
          ...
          : input.hostKeyPrompted
            ? details(text("connectStages.hostKeyPromptDetail"))
            : undefined,
    });
  }
          ...
          : connected
            ...
            : failed
              ...
              : readyToConnect
                ? "active"
                : "pending",
      detail: failureLayer === "ssh" ? details(failureMessage) : undefined,
    });
  }
  ...
  if (failed && !stages.some((entry) => entry.state === "failed")) {
    const current = stages.find(
      (entry) => entry.state === "active" || entry.state === "pending",
    );
    if (current) {
      current.state = "failed";
      current.detail = details(failureMessage) ?? current.detail;
    }
  }
 ...
 * 정한다: 코어는 통로를 열기 전에 `ssh-tunnel-gateway`, 열린 뒤에 `ssh-tunnel-open` 을 보낸다.
 */
  ...
  return [
    {
      ...
      group: "host",
      label: text("connectStages.tunnelGateway", { label }),
      state: openReported
        ? "done"
        : gatewayActive
          ? "active"
    ...
    },
    {
      id: "ssh-tunnel",
      group: "host",
      label: text("connectStages.tunnelOpen"),
...
function resolveTargetStage(
  ...
  const stage: ConnectionStage = {
    id: "tailscale-target",
    group: "tailscale",
    label: text("connectStages.tailscaleTarget", { target }),
    state: "pending",
  };
  ...
  if (!peer) {
    ...
    const via = peers.find((candidate: TailnetStagePeer) =>
      (candidate.routes ?? []).some((route: string) =>
        addressWithinRoute(wanted, route),
      ),
    );
    if (via) {
      const peerName = via.hostName || via.dnsName;
      return {
        ...stage,
        state: "done",
        detail: details(
          peerName
            ? text("connectStages.tailscaleTargetViaSubnet", { peer: peerName })
            : // 이름이 없는 기기가 대역을 광고하는 경우가 있다. "이름 없는 기기" 라고 쓰는 대신
              // 그 사실을 말하는 문구를 따로 둔다 — 두 문구를 이어 붙이는 것은 앱이 한다.
              text("connectStages.tailscaleTargetViaSubnetRouter"),
        ),
      };
    }
    ...
    if (looksLikeIpAddress(wanted)) {
      return {
        ...stage,
        state: "warn",
        detail: details(text("connectStages.tailscaleTargetNotInTailnet")),
      };
    }
    return {
      ...stage,
      state: "failed",
      detail: details(text("connectStages.tailscaleTargetMissing")),
    };
  }
  ...
  if (peer.direct) {
    return {
      ...stage,
      state: "done",
      detail: details(text("connectStages.tailscaleTargetDirect"), traffic),
    };
  }
  if (peer.relay) {
    return {
      ...
      state: "done",
      detail: details(
        text("connectStages.tailscaleTargetRelay", { relay: peer.relay }),
        traffic,
      ),
    };
  }
  return {
    ...stage,
    state: "failed",
    detail: details(text("connectStages.tailscaleTargetNoPath"), traffic),
  };
}
...
function formatTraffic(
  txBytes?: number,
  rxBytes?: number,
): ConnectionStageText | undefined {
  if (txBytes === undefined && rxBytes === undefined) {
    return undefined;
  }
  return text("connectStages.tailscaleTargetTraffic", {
