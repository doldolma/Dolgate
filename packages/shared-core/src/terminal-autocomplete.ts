import type { TerminalAutocompleteSnapshot } from './models';
...
export function applyTerminalInput(
  ...
  for (let index = 0; index < data.length; ) {
    ...
    if (rest.startsWith('\x1b[200~') || rest.startsWith('\x1b[201~')) {
      next.ambiguous = true;
      index += 6;
      continue;
    }
    ...
    if (
      rest.startsWith('\x1b[A') ||
      rest.startsWith('\x1b[B') ||
      rest.startsWith('\x1bOA') ||
      rest.startsWith('\x1bOB')
    ) {
    ...
    if (rest.startsWith('\x1b[3~')) {
      if (next.cursor < next.value.length) {
        next.value =
          next.value.slice(0, next.cursor) + next.value.slice(next.cursor + 1);
      }
      index += 4;
      continue;
    }
    if (
      ...
      next.cursor = 0;
      index += rest.startsWith('\x1b[1~') ? 4 : 3;
      continue;
    }
    if (
      ...
      index += rest.startsWith('\x1b[4~') ? 4 : 3;
      continue;
    }
    if (rest.startsWith('\x1b')) {
      // Unknown CSI/SS3/Alt input changes readline state in ways we cannot
      // reconstruct locally. Consume the escape sequence as one unit so its
      // printable bytes never become a fake command, and suppress suggestions
      // until the next OSC prompt boundary restores an exact buffer.
      const sequence =
        /^\x1b\[[0-?]*[ -/]*[@-~]/.exec(rest)?.[0] ??
        /^\x1bO./.exec(rest)?.[0] ??
        '\x1b';
    ...
    switch (char) {
      ...
      case '\b':
        if (next.cursor > 0) {
          next.value =
            next.value.slice(0, next.cursor - 1) +
      ...
      default:
        if (codePoint >= 0x20 && codePoint !== 0x7f) {
          next.value =
            next.value.slice(0, next.cursor) +
            char +
            next.value.slice(next.cursor);
...
export function getTerminalAutocompleteSuggestions(
  ...
  if (!context.suppressHistory) {
    for (const entry of lineKeys) {
      ...
      let score = sessionStat
        ? SCORE_WEIGHTS.sessionBase
        : SCORE_WEIGHTS.historyBase;
      if (sessionStat) {
        score +=
        ...
        score +=
          SCORE_WEIGHTS.recency * ((fileStat.lastIndex + 1) / historyLength);
      }
