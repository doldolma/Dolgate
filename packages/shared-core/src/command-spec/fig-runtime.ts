...
/**
 * 화면에 보여줄 이름. Fig 의 `displayName` 이 있으면 그것이고, 없으면 없다.
 *
 * 넣는 값은 `insertValue ?? name` 이라 이것과 다를 수 있다 — 그 둘을 가르는 것이 이 함수의
 * 존재 이유다.
 */
export function figSuggestionDisplayName(
...
export function figSuggestionsToCompletions(
  ...
  before: string,
  base: string,
): Array<{ insertText: string; displayText?: string; description?: string }> {
  const out: Array<{
    insertText: string;
    displayText?: string;
    description?: string;
  ...
  for (const suggestion of suggestions) {
    ...
    seen.add(insertText);
    // `displayName` 은 부가정보가 아니라 **이름 대신 보여줄 이름**이다(`Format`·`restore[exact]`
    // 처럼 넣는 값과 아주 다른 것도 있다). 넣는 값과 같을 때만 굳이 싣지 않는다.
    const displayName = figSuggestionDisplayName(suggestion);
    const entry: {
      insertText: string;
      displayText?: string;
      description?: string;
