...
const FOLDER_COMMANDS = new Set(['cd', 'pushd', 'mkdir', 'rmdir']);
...
export function resolveDynamicCompletion(
  ...
  const pathLike = token.includes('/') || token.startsWith('~');
  ...
  // Mobile deliberately does not bundle the full Fig catalogue. Keep common
  // filesystem commands useful even without a spec; explicit specs still win
  // above and can narrow the argument more accurately.
  const leadingCommand = completedTokens[0] ?? '';
  if (
    !spec &&
    completedTokens.length === 1 &&
    PATH_COMMANDS.has(leadingCommand)
  ) {
    return {
      kind: 'path',
      before,
      dir,
      base,
      foldersOnly: FOLDER_COMMANDS.has(leadingCommand),
    };
  }
